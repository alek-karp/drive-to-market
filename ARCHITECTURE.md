...
5. `POST /api/compose-textures` validates a `WrapDesign` and calls `lib/composeTextures.ts` to generate per-part PNG textures plus first-class hood logo and trunk CTA graphic URLs.
